...
func main() {
	...
	inputFile := flag.String("input", "", "Path to input Excel file (.xlsx)")
	outputFile := flag.String("output", "output.txt", "Path to output text file")
	keyCol := flag.String("key-col", "A", "Column letter for grouping (e.g. A, B, C)")
	valCol := flag.String("val-col", "B", "Column letter for values to extract (e.g. A, B, C)")
	limit := flag.Int("limit", 200, "Number of values to extract per group")
	shuffle := flag.Bool("shuffle", false, "Randomize selection within each group")
	...
	// Sort segments for deterministic output order
